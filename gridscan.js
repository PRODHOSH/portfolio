...
export function initGridScan(container, options = {}) {
    ...
    let tiltCurrent = 0, tiltVel = 0, yawCurrent = 0, yawVel = 0;
    ...
    function smoothDampFloat(current, target, vel, smoothTime, maxSpeed, dt) {
        smoothTime = Math.max(0.0001, smoothTime);
        const omega = 2 / smoothTime;
        const x = omega * dt;
        const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
        ...
        return { value: target + (change + temp) * exp, velocity: vel };
    }
    ...
    function animate() {
        ...
        const tiltResult = smoothDampFloat(tiltCurrent, 0, tiltVel, smoothTime, maxSpeed, dt);
        tiltCurrent = tiltResult.value;
        tiltVel = tiltResult.velocity;

        const yawResult = smoothDampFloat(yawCurrent, 0, yawVel, smoothTime, maxSpeed, dt);
        yawCurrent = yawResult.value;
        yawVel = yawResult.velocity;
